...
export default function CardShop(product: Partial<CartI>) {
  ...
  const { products, complete } = useSelector((state: RootState) =>
    auth.user ? state.apiCart : state.cart,
  );
  ...
  useEffect(() => {
    ...
    if (auth.user) {
      ...
      const selectorLocalAmount = document.getElementById(
        `localAmount${product.idProduct}`,
      ) as HTMLInputElement;
      if (
        currentProduct?.sizeCart?.stock &&
        parseInt(selectorLocalAmount.value) > currentProduct.sizeCart.stock
      ) {
        Swal.fire({
        ...
        product.idProduct &&
          dispatch(
            updateLocalCart({
              ...
              sizes: { stock: currentProduct.sizeCart.stock },
            }),
          );
      }
    }
  ...
  const updateAmount = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    product.idProduct &&
      dispatch(updateLocalCart({ method: e.currentTarget.name, id: product.idProduct, sizes: {} }));
  };
  ...
  const updateApiAmount = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (product.quantity !== apiQuantity) {
      if (
        product?.sizeCart?.stock !== undefined &&
        apiQuantity !== undefined &&
        product.sizeCart.stock >= apiQuantity
      ) {
        await dispatch(
          updateApiUserCart({
            idUser: auth.id,
  ...
  const updateApiSize = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (findedApiSize?.stock === 0) {
      Swal.fire({
        text: "Can't change your size cause there is not more stock available",
        icon: 'warning',
      });
    } else {
      await dispatch(
  ...
  const verifyStock = () => {
    const selectorAmount = document.getElementById(
      `apiAmount${product.idProduct}`,
    ) as HTMLInputElement;
    if (
      currentProduct?.sizeCart?.stock &&
      parseInt(selectorAmount.value) > currentProduct.sizeCart.stock
    ) {
      Swal.fire({
        text: 'We will update your amount cause you have more than stock available',
        icon: 'warning',
      ...
      dispatch(
        updateApiUserCart({
          idUser: auth.id,
          idProduct: product.idProduct,
          quantity: currentProduct.sizeCart.stock,
        }),
      );
    }
  };
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (!auth.user) {
          ...
          complete && Swal.fire('Deleted!', 'Your product has been deleted.', 'success');
        }
      }
    });
  };
          ...
          <Grid container>
            ...
            <Grid
              ...
              <Link
                ...
                to={`${PublicRoutes.products}/${product.idProduct}`}>
                {product.name}
              </Link>
            </Grid>
            <Grid item xs={1}>
            ...
            <Grid item xs={1}>
              ...
              <Box display={auth.user ? 'flex' : 'inherit'} gap={1}>
                <select
                  ...
                  onChange={updateSize}>
                  <option id='currentSize' value='currentSize' disabled>
                    sizes
                  </option>
                  {product?.size?.map(s => (
                ...
                {auth.user && (
                  <Button
                    ...
                    name='apiUpdate'
                    onClick={updateApiSize}>
                    <Update sx={{ width: 'fit-content' }} />
                  </Button>
                )}
              </Box>
            </Grid>
            ...
            {!auth.user && (
              <Grid item xs={2} display={'flex'} alignItems='center' justifyContent='center'>
                ...
                <input
                  id={`localAmount${product.idProduct}`}
                  disabled
                ...
                <button
                  name='increase'
                  disabled={
                    product.quantity && product.sizeCart?.stock
                      ? product.quantity >= product.sizeCart.stock
                      : false
                  }
                  style={{ width: '25px' }}
            ...
            {auth.user && (
              <Grid item xs={2} display='flex' gap={1} alignItems='center' justifyContent='center'>
                <input
                  id={`apiAmount${product.idProduct}`}
                  type='number'
                  title="can't exceed stock"
                  value={apiQuantity}
                  style={{ width: '20px', textAlign: 'center' }}
            ...
            <Grid item xs={2}>
              <Button
                ...
                onClick={e => product.idProduct && deleteProduct(e, product.idProduct)}>
                Delete
              </Button>
            </Grid>
          </Grid>
