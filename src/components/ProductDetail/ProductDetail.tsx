...
export default function ProductDetail() {
  ...
  const cartProduct: CartI = {
    ...
    sizeCart: user
      ? undefined
      : shoe.details?.sizes && shoe.details.sizes[shoe.details?.sizes?.length - 1],
      ...
      <Container maxWidth='xl'>
        <Grid container spacing={1}>
          ...
          <Grid item xs={6}>
            ...
            {findedProduct && (
              <Box>
                <Typography variant='h5' component='h2' pt={1}>
                  Current Selected Size: {findedProduct?.sizeCart?.size}
                </Typography>
                <Typography variant='h5' component='h2' pt={1}>
                  Current Avalable Stock: {maxGralStock}
                </Typography>
              </Box>
            )}
            {!findedProduct && (
              <Sizes
                sizes={shoe?.details?.sizes}
                updateSizes={(newValue: string) => setSizeValue({ value: newValue })}
              />
            )}
            <Box
              mt={2}
